...
class _SweatshirtSizeState extends State<SweatshirtSize> {
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          centerTitle: true,
          title: const Text(
            "SWEATSHIRT SIZE", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold,fontFamily: 'CeraPro', letterSpacing: 3.5),),
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      Map<String, dynamic> measurements = sizeData['measurements'];
  ...
  void _showInputDialog(BuildContext context, String size, Map<String, dynamic> measurements) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text("Enter Measurements for $size", style: TextStyle(fontFamily: 'CeraPro')),
          content: Column(
          ...
          actions: [
            TextButton(
              ...
              child: const Text("Cancel", style: TextStyle(fontFamily: 'CeraPro', fontSize: 18),),
            ),
            TextButton(
              onPressed: () async {
                Navigator.of(context).pop();
                Map<String, double?> actualSizes = {
                  for (var key in controllers.keys) key: double.tryParse(controllers[key]!.text)
                };

                if (actualSizes.values.any((size) => size == null)) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("Please enter valid measurements.", style: TextStyle(fontFamily: 'CeraPro'),)),
                  );
                  return;
                }
                ...
                if (chestMeasurement != null) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("Chest measurement is required.", style: TextStyle(fontFamily: 'CeraPro'),)),
                  );
                }
              },
              ...
              child: const Text("Confirm", style: TextStyle(fontFamily: 'CeraPro'),),
            ),
